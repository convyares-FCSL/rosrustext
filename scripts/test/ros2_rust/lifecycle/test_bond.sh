...
state_label() {
  timeout "${ROS2_TIMEOUT}s" ros2 lifecycle get "${NODE_NAME}" | awk '{print $1}'
}
...
ensure_unconfigured() {
  ...
  case "$s" in
    ...
    Active)
      ...
      ;;
    Finalized)
      fail "node is Finalized; restart the process/node before running this script"
      ;;
    *)
      fail "unknown state label: '$s'"
      ;;
  esac
}
...
if command -v setsid >/dev/null 2>&1; then
  if [[ "${BOND_VERBOSE:-0}" == "1" ]]; then
    setsid cargo run &
  else
    echo "info: building/running demo node (set BOND_VERBOSE=1 for full cargo output)"
...
echo "== activate (expect active=true) =="
ros2_lifecycle_set activate
expect_bond_active "true"

echo "== deactivate (expect active=false) =="
ros2_lifecycle_set deactivate
expect_bond_active "false"
