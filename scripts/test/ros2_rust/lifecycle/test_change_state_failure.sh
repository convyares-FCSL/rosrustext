...
if command -v setsid >/dev/null 2>&1; then
  ROSRUSTEXT_RCLRS_CHANGE_STATE_DELAY_MS="${CHANGE_STATE_DELAY_MS}" \
    ROSRUSTEXT_RCLRS_TRANSITION_RESULT_CONFIGURE="failure" \
...
rclpy.init()
node = Node("rosrustext_change_state_failure_checker")
...
if cur == "Active":
    ...
    if call_change_state(Transition.TRANSITION_CLEANUP) is None:
...
cur = get_state_label()
if cur != "Unconfigured":
    print(f"error: expected Unconfigured after failure, got {cur}", file=sys.stderr)
    sys.exit(1)
...
print("ok: failure path emitted one event; state unchanged after completion")
rclpy.shutdown()
PY
