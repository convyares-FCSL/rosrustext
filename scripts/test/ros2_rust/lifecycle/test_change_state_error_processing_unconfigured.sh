...
if command -v setsid >/dev/null 2>&1; then
  ROSRUSTEXT_RCLRS_CHANGE_STATE_DELAY_MS="${CHANGE_STATE_DELAY_MS}" \
    ROSRUSTEXT_RCLRS_TRANSITION_RESULT_ACTIVATE="error" \
    ROSRUSTEXT_RCLRS_ON_ERROR_RESULT_ACTIVATE="success" \
...
rclpy.init()
node = Node("rosrustext_change_state_error_unconfigured_checker")
...
def call_change_state(transition_id, timeout_s=2.0):
...
if cur not in ("Unconfigured", "Inactive", "Active"):
    ...
    sys.exit(1)
if cur == "Active":
    if call_change_state(Transition.TRANSITION_DEACTIVATE) is None:
...
if cur != "Inactive":
    if call_change_state(Transition.TRANSITION_CONFIGURE) is None:
        print("error: failed to configure before test", file=sys.stderr)
        sys.exit(1)
...
resp = future.result()
if resp is None:
    print("error: change_state service call failed (no response)", file=sys.stderr)
    sys.exit(2)
if not resp.success:
    print("error: change_state was not accepted", file=sys.stderr)
    sys.exit(3)
...
cur = get_state_label()
if cur != "Inactive":
    print(f"error: expected Inactive immediately after change_state, got {cur}", file=sys.stderr)
    sys.exit(1)

if not wait_for_state("Unconfigured", state_wait_timeout):
    print(f"error: state did not converge to Unconfigured within {state_wait_timeout}s", file=sys.stderr)
    sys.exit(1)
...
if event_msg["msg"] is None:
    ...
    sys.exit(1)
if event_msg["msg"].transition.label != "activate":
...
print("ok: error->ErrorProcessing->Unconfigured emitted one event")
rclpy.shutdown()
PY
