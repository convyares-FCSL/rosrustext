...
for svc in get_state change_state get_available_states get_available_transitions; do
  ros2_service_list | grep -qx "${NODE_NAME}/${svc}" || \
    fail "missing service: ${NODE_NAME}/${svc}"
done
