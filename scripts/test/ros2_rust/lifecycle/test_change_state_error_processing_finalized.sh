...
if command -v setsid >/dev/null 2>&1; then
  ROSRUSTEXT_RCLRS_CHANGE_STATE_DELAY_MS="${CHANGE_STATE_DELAY_MS}" \
    ...
    ROSRUSTEXT_RCLRS_ON_ERROR_RESULT_ACTIVATE="failure" \
...
rclpy.init()
node = Node("rosrustext_change_state_error_finalized_checker")
...
if not wait_for_state("Finalized", state_wait_timeout):
    print(f"error: state did not converge to Finalized within {state_wait_timeout}s", file=sys.stderr)
    sys.exit(1)
...
print("ok: error->ErrorProcessing->Finalized emitted one event")
rclpy.shutdown()
PY
