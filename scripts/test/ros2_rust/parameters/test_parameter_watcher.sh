...
EXAMPLE_DIR="${DEV_WS_ROOT}/examples/ros2_rust_parameters_watcher"
NODE_NAME="/rosrustext_parameters_watcher"
...
force_kill_node_processes() {
  if command -v pkill >/dev/null 2>&1; then
    pkill -TERM -f "rosrustext_parameters_watcher" >/dev/null 2>&1 || true
    sleep 1
    pkill -KILL -f "rosrustext_parameters_watcher" >/dev/null 2>&1 || true
  fi
}
...
if command -v setsid >/dev/null 2>&1; then
  if [[ "${EXAMPLE_VERBOSE:-0}" == "1" ]]; then
    setsid cargo run --manifest-path "${EXAMPLE_DIR}/Cargo.toml" --bin parameter_watcher &
  else
    setsid cargo run --manifest-path "${EXAMPLE_DIR}/Cargo.toml" --bin parameter_watcher --quiet >"${NODE_LOG}" 2>&1 &
  fi
  NODE_PGID=$!
else
  if [[ "${EXAMPLE_VERBOSE:-0}" == "1" ]]; then
    cargo run --manifest-path "${EXAMPLE_DIR}/Cargo.toml" --bin parameter_watcher &
  else
    cargo run --manifest-path "${EXAMPLE_DIR}/Cargo.toml" --bin parameter_watcher --quiet >"${NODE_LOG}" 2>&1 &
  fi
fi
NODE_PID=$!
...
echo "== set timer_period_s =="
timeout "${ROS2_TIMEOUT}s" ros2 param set "${NODE_NAME}" timer_period_s 0.2 >/dev/null || \
  fail "ros2 param set timed out"

wait_for_log_token "WATCHER_APPLIED" "${ROS2_TIMEOUT}" || \
  fail "expected WATCHER_APPLIED log token"
