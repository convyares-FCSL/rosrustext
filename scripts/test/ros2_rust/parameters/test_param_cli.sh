...
list_out="$(timeout "${ROS2_TIMEOUT}s" ros2 param list "${NODE_NAME}")" || \
  fail "ros2 param list timed out"

echo "${list_out}" | grep -q "timer_period_s" || \
  fail "expected timer_period_s in param list"

echo "${list_out}" | grep -q "max_count" || \
  fail "expected max_count in param list"

types_out="$(timeout "${ROS2_TIMEOUT}s" ros2 service call \
  "${NODE_NAME}/get_parameter_types" \
  rcl_interfaces/srv/GetParameterTypes {names:[timer_period_s]})" || \
  fail "get_parameter_types service call timed out"

echo "${types_out}" | grep -q "types" || \
  fail "get_parameter_types response missing types"

get_out="$(timeout "${ROS2_TIMEOUT}s" ros2 param get "${NODE_NAME}" timer_period_s)" || \
  fail "ros2 param get timed out"

echo "${get_out}" | grep -qi "double" || \
  fail "expected Double type for timer_period_s"

describe_out="$(timeout "${ROS2_TIMEOUT}s" ros2 param describe "${NODE_NAME}" timer_period_s)" || \
  fail "ros2 param describe timed out"

echo "${describe_out}" | grep -q "timer_period_s" || \
  fail "describe output missing timer_period_s"
