...
use rclrust_core::lifecycle::{
    drive, shutdown_ros_id_for_state, transition_from_ros_id, 
    ActivationGate, LifecycleCallbacks, State, available_transitions};

pub mod managed_publisher;
...
///
/// The transport layer (roslibrust) will map real ROS messages into these.
...
///
/// Transport maps real ROS messages into these.
...
impl LifecycleNode {
    ...
    pub(crate) fn handle_get_available_transitions( &self, _req: get_available_transitions::Request ) -> get_available_transitions::Response {
...
mod tests {
    ...
    fn change_state_handler_success() {
        ...
        let resp = node.handle_change_state(crate::lifecycle::change_state::Request {
    ...
    fn get_state_reports_current_state() {
        ...
        let resp = node.handle_get_state(crate::lifecycle::get_state::Request);
    ...
    fn get_available_transitions_reports_active_state() {
        ...
        // Configure + activate
        node.request_transition_ros(rclrust_core::lifecycle::ros_ids::TRANSITION_CONFIGURE).unwrap();
        node.request_transition_ros(rclrust_core::lifecycle::ros_ids::TRANSITION_ACTIVATE).unwrap();

        let resp = node.handle_get_available_transitions(
            crate::lifecycle::get_available_transitions::Request,
        );
    ...
    #[test]
    fn run_if_active_blocks_when_inactive() {
        let gate = rclrust_core::lifecycle::ActivationGate::new();
        let mut ran = false;

        let ok = crate::lifecycle::run_if_active(&gate, || ran = true);
    ...
    #[test]
    fn run_if_active_runs_when_active() {
        let gate = rclrust_core::lifecycle::ActivationGate::new();
        gate.activate();
