...
use rosrustext_roslibrust::lifecycle::{
    ros_state_id, ros_transition_id, shutdown_ros_id_for_state, state_from_ros_id, transition_from_ros_id,
};
...
fn transition_graph_matches_core_and_ros_mappings() {
    ...
    for edge in &graph.transitions {
        let ros_id =
            ros_transition_id(edge.start, edge.transition).expect("ROS transition id should exist for graph edge");
    ...
    for state in [CoreState::Unconfigured, CoreState::Inactive, CoreState::Active] {
