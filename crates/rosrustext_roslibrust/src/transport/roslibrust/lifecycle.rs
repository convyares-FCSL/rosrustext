...
impl LifecycleService {
        ...
        match work {
            Ok(work) => {
                ...
                spawn_transition_task(move || {
                    ...
                    let on_error = if result == CallbackResult::Error { Some(work.callbacks.on_error()) } else { None };

                    let final_state =
                        match finish_with_error_handling(work.intermediate_state, work.transition, result, on_error) {
                            Ok(state) => state,
                            Err(err) => {
                                log_core_error(err);
                ...
                dtos::change_state::Response { success: true, message: "transition accepted".to_string() }
            }
            Err(e) => dtos::change_state::Response { success: false, message: format!("transition failed: {e}") },
        }
    }
    ...
    pub fn handle_get_available_transitions(
        &self, req: dtos::get_available_transitions::Request,
    ...
    pub fn handle_get_available_states(
        &self, req: dtos::get_available_states::Request,
...
fn run_transition_callback(
    transition: Transition, callbacks: &mut dyn crate::lifecycle::LifecycleCallbacks,
) -> CallbackResult {
    match transition {
...
mod tests {
    ...
    fn change_state_dto_maps_and_reports_success() {
        ...
        let req = dtos::change_state::Request { transition_id: crate::lifecycle::ros_ids::TRANSITION_CONFIGURE };
    ...
    fn change_state_dto_rejects_unsupported_transition_id() {
        ...
        let resp = svc.handle_change_state(dtos::change_state::Request { transition_id: u8::MAX });
        assert!(!resp.success);
        assert!(!resp.message.is_empty());
    }
